...
__global__ void k_debug_lookup_int(const int *gathered_stack4d) {
  for (int i = 0; i < 64 * 3; ++i) {
    if (!(i % 4)) printf("\n");
    if (!(i % 16)) printf("------------\n");
    if (!(i % 64)) printf("------------\n");
...
void debug_kernel_int(const int *tmp) {
...
__global__ void k_gather_cubes(const uchar *__restrict img, const uint3 size,
                               const bm4d_gpu::Parameters params,
                               const uint3float1 *__restrict d_stacks, const uint array_size,
...
void gather_cubes(const uchar *__restrict img, const uint3 size, const uint3 tsize,
                  const bm4d_gpu::Parameters params, uint3float1 *&d_stacks, uint *d_nstacks,
                  float *&d_gathered4dstack, uint &gather_stacks_sum,
                  const cudaDeviceProp &d_prop) {
  ...
  checkCudaErrors(
      cudaMalloc((void **)&d_stacks_compacted, sizeof(uint3float1) * gather_stacks_sum));
  ...
  thrust::copy_if(dt_stacks, dt_stacks + params.maxN * tsize.x * tsize.y * tsize.z,
  ...
  checkCudaErrors(cudaMalloc((void **)&d_gathered4dstack,
...
__global__ void dct3d(float *d_gathered4dstack, int patch_size, uint gather_stacks_sum) {
  for (int cuIdx = blockIdx.x; cuIdx < gather_stacks_sum; cuIdx += blockDim.x * gridDim.x) {
    ...
    float tmp = dct_coeff[y][0] * cube[z][0][x] + dct_coeff[y][1] * cube[z][1][x] +
                dct_coeff[y][2] * cube[z][2][x] + dct_coeff[y][3] * cube[z][3][x];
    __syncthreads();
    cube[z][y][x] = tmp;
    __syncthreads();
    tmp = dct_coeff_T[0][x] * cube[z][y][0] + dct_coeff_T[1][x] * cube[z][y][1] +
          dct_coeff_T[2][x] * cube[z][y][2] + dct_coeff_T[3][x] * cube[z][y][3];
    __syncthreads();
    cube[z][y][x] = tmp;
    __syncthreads();
    // Grab Z vector
    ...
    __syncthreads();
    cube[z][y][x] = z_vec[0] * dct_coeff[z][0] + z_vec[1] * dct_coeff[z][1] +
                    z_vec[2] * dct_coeff[z][2] + z_vec[3] * dct_coeff[z][3];
    __syncthreads();
...
__global__ void idct3d(float *d_gathered4dstack, int patch_size, uint gather_stacks_sum) {
  for (int cuIdx = blockIdx.x; cuIdx < gather_stacks_sum; cuIdx += blockDim.x * gridDim.x) {
    ...
    __syncthreads();
    cube[z][y][x] = z_vec[0] * dct_coeff_T[z][0] + z_vec[1] * dct_coeff_T[z][1] +
                    z_vec[2] * dct_coeff_T[z][2] + z_vec[3] * dct_coeff_T[z][3];
    __syncthreads();
    float tmp = dct_coeff_T[y][0] * cube[z][0][x] + dct_coeff_T[y][1] * cube[z][1][x] +
                dct_coeff_T[y][2] * cube[z][2][x] + dct_coeff_T[y][3] * cube[z][3][x];
    __syncthreads();
    cube[z][y][x] = tmp;
    tmp = dct_coeff[0][x] * cube[z][y][0] + dct_coeff[1][x] * cube[z][y][1] +
          dct_coeff[2][x] * cube[z][y][2] + dct_coeff[3][x] * cube[z][y][3];
    __syncthreads();
    cube[z][y][x] = tmp;
    __syncthreads();
    d_gathered4dstack[idx] = cube[z][y][x];
...
struct is_computed_weight {
  __host__ __device__ bool operator()(const float x) { return (x < 1.0); }
};
...
__global__ void k_aggregation(float *d_denoised_volume, float *d_weights_volume, const uint3 size,
                              const uint3 tsize, const float *d_gathered4dstack,
...
__global__ void k_normalizer(float *d_denoised_volume, const float *__restrict d_weights_volume,
                             const uint3 size) {
